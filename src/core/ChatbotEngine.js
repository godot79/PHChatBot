...
class ChatbotEngine {
   ...
   * Uses verified flag to route:
   *  - Unverified -> INTRO
   *  - Verified:
   *      • If current state is BOOKING_METHOD_OPTIONS → show booking method menu
   *      • Else → set BOOK_MANAGE_OPTIONS and show verified main menu
   *
  ...
  */
  async renderMainMenu(session) {
    ...
    if (context && context.region) {
      ...
      const code = String(context.region).toUpperCase();
      regionCode = regionLabels[code] ? code : 'SG';
    ...
    // If verified, keep your existing verified main menu
    if (session.verified) {
      return (
        `${region}` +
    ...
    // Unverified (Intro) menu per request:
    // 1 Existing Clients: Book or Manage Appointments → verification flow
    // 2 New Clients: Book or Manage Appointments      → registration flow
    // 3 View Fees
  ...
  */
  async handleIntroState(session, message) {
    ...
    // Determine regionCode for conditional options
    let regionCode = 'SG';
    try {
    ...
    // 1 Existing Clients -> Verify (unchanged handler)
    ...
    // 2 New Clients -> Registration flow (reuse existing register handler)
    if (text === '2' || (text.includes('new') && text.includes('client'))) {
      await this.sessionManager.updateSession(session.id, {
    ...
    // 4 View Locations (only for non-SG)
    if ((text === '4' || text.includes('location')) && regionCode !== 'SG') {
      await this.sessionManager.updateSession(session.id, {
  ...
  async handleVerifyState(session, message) {
    ...
    if (['0', 'menu', 'back'].includes(text)) {
      // Clear transient verification flags only
      const cleared = { ...data };
    ...
    // Always clear transient verification flags regardless of outcome
    const clearedData = { ...data };
    delete clearedData.awaiting_email;

    if (patient) {
      try {
        // Persist email in context for downstream email composers
        if (typeof this.saveEmailToSessionContext === 'function') {
          await this.saveEmailToSessionContext(session, email);
        }
        // Optionally persist into session.data for composers that read data.email
        if (typeof this.saveEmailToSessionData === 'function') {
      ...
      // Persist only what should survive: verified + patient_id + next state
      await this.sessionManager.updateSession(session.id, {
        verified: true,
      ...
      // Show verified main menu (don’t carry forward VERIFY state)
  ...
  async handleConfirmBookingState(session, message) {
    ...
    if (text === 'yes') {
      ...
      if (!patient_id) {
        ...
        await this.sessionManager.updateSession(session.id, { verified: false, conversation_state: this.STATES.INTRO });
  ...
  async handleViewFeesState(session, message) {
    try {
      ...
      // Do not force INTRO; let goToInteractiveMenu set and render appropriate menu
      await this.sessionManager.updateSession(session.id, {});
      return `${body}\n\n` + await this.goToInteractiveMenu(session);
    } catch (e) {
  ...
  async handleViewLocationsState(session, message) {
    ...
    // Do not force INTRO; let goToInteractiveMenu decide and render
    await this.sessionManager.updateSession(session.id, {});
  ...
  */
  ...
  async handleConfirmCancelState(session, message) {
    ...
    try {
      const sessionRow = await this.sessionManager.getSession(session.id);
      const appt2 = data.selected_cancel_appt || {};
      const payloadAppt = {
        id: appt2.id,
        starts_at: appt2.starts_at || appt2.appointment_start || appt2.slot,
        appointment_type: appt2._appointment_type_display || appt2.appointment_type_name || appt2.appointment_type,
        practitioner: appt2._practitioner_display || appt2.practitioner_name || appt2.practitioner,
        clinic: appt2._business_display || appt2.business_name || appt2.clinic,
  ...
  async handleConfirmRescheduleState(session, message) {
    ...
    if (/^\d+$/.test(text)) {
      ...
      if (result && result.success) {
        try {
          ...
          this.logger.error('Reschedule success email send error:', err);
        }
    ...
    // Initial render (or invalid input)
    const slotList = formatPaginatedList({
      items: availableTimes,
      formatFn: (s, i) => `${i}. ${new Date(s.starts_at || s.appointment_start || s.slot).toLocaleString()}`,
      page: slot_page,
