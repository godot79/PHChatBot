...
if (/^p(rev)?$/i.test(text)) {
  ...
  async handleBookSoonest(session, message) {
    ...
    const buildAvailablePhysiosForTypeName = async (typeNorm) => {
      ...
      // One practitioner's fetch failing (timeout, 429, etc.) must not discard
      // every other practitioner's already-fetched types — Promise.all would
      // reject the whole batch on a single rejection.
      const settledTypes = await Promise.allSettled(
        allPractitioners.map(p => this.clinikoAPI.getAppointmentTypes({ practitioner_id: p.id }))
      );
      const hadTypeFetchFailure = settledTypes.some(s => s.status === 'rejected');
      const phys = allPractitioners.filter((p, i) =>
        settledTypes[i].status === 'fulfilled' &&
        (settledTypes[i].value || []).some(t => normName(t.name) === typeNorm)
      );
      // Filter by slot availability — all slot checks in parallel
      const flags = await Promise.all(
        phys.map(p => practitionerHasSlotsForTypeName(p, typeNorm))
      );
      const result = phys.filter((_, i) => flags[i]);
      return hadTypeFetchFailure ? _markPartial(result) : result;
    };
